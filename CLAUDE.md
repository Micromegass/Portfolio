...
- **Prices are published for simple work only** (`/pricing/`, since 2026-08-30): fixed €1,200 for a one-pager, €1,900 for up to six frontend-only pages, care from €39/month. Everything else — backends, web apps, the AI assistant, shops — has deliberately **no list price** and is a "talk to us". A tier's `price: null` is what renders that state, so a figure can never ship for an offer we have not committed to. No blog. Contact is email only.
...
- **Scope rules live in `backend/src/prompt.ts`:** it may repeat the published prices verbatim but never estimates an unpriced project, negotiates, discounts, converts currency or quotes an hourly rate; never commits the studio to anything; never claims client outcomes; says "I don't know" and hands off to email instead of guessing; and treats instructions inside visitor messages as text. These rules are the product — change them deliberately, and keep rule 2 in step with `/pricing/`.
...
- 2026-07-10: Interview + strategy approved. Contact = email only; no pricing (**superseded 2026-08-30**, see below); no blog; German legal pages required; noindex until the real domain is live.
...
- 2026-08-30: **Homepage now carries the "Current builds" band** (client projects in flight; Trazo is filtered out there — in-house, and the filter keeps the 2-up grid even). **Harmonia Baila added as the fifth build** — bilingual ES/EN dance-academy site in Medellín (salsa/bachata, kids' classes, WhatsApp trial-class booking), GH Pages preview linked, status "with the client for review". **Work page case band labeled** ("Live client work" SectionHead + LIVE chip on each case card) after Axel said the two cards were "too in your face" with no hint they are real businesses online; case card titles pinned to `--text-lg` regardless of heading level. **Physio slogan sobered** — "A practice that explains itself clearly" → "The website of a therapy practice in Fürth" (Axel: nothing fancy, don't overstate; long form clipped on DE mobile at h1 scale, so card and page share the short form). **Scroll feel fixed**: native `scroll-behavior: smooth` now switches off while Lenis runs (`html.lenis` override in global.css) — the two easings fighting was the notchy scroll — Lenis moved to `duration: 1.2` + `anchors: true`, and GSAP's default lag smoothing restored (no more `lagSmoothing(0)`) so heavy frames can't become scroll jumps.

- 2026-08-30: **Pricing published — this reverses the 2026-07-10 "no pricing" decision.** New page at `/pricing/` · `/de/preise/` · `/es/precios/` (`PricingPage.astro`, `pricing` section in the three data files). Numbers came from market research over ~30 sources: German freelancer/micro-studio price pages cluster at €1,450–2,490 for this work and the published freelancer floor is "ab €1.000", so **Axel chose €1,200 / €1,900 deliberately below market** to win simple-website clients; the research floor of €1,450 is on record if he later wants to move up. Care plan €39/month is inside the German €39–99 band and **cancellable monthly** where the market locks 6–12 months. Prices are **EUR in all three languages** — the ES page adds one line inviting Colombian visitors to talk, because €1,200 ≈ 4.3M COP is upper-agency money in Medellín and a second price list would create two public realities. **Consequences that must stay in step:** `prompt.ts` rule 2 (assistant may now quote the listed figures, nothing more), the `## Pricing` block in `build-knowledge.mjs` (it cherry-picks — a new `SiteContent` section does *not* flow in on its own), `llms.txt.ts` (the "No pricing is published" claim is gone), the relaunch FAQ in all three languages, and `Seo.astro` (FAQPage + Service/Offer schema; offers are derived from the price strings, and a `null` price contributes no offer). Header burger moved 920px → 1024px with a tightening step at 1240px: seven nav items wrapped the row onto two lines, and the DE/ES CTA label wrapped inside its own pill.
- 2026-08-30: Fixed a silent bug in `scripts/build-knowledge.mjs` — the builds loop read `p.url`, but the data shape is `preview: { url, label }`, so **no preview link had ever reached the assistant**.
